...
# (Run install.packages only once on the cluster, then you can comment it out)
...
# 1. FULL IMAGE SIZE   #
########################
# NOTE: This chunk depends on heaght_width_FULL.csv,
# which is created by the Python script full_image_size_cropped_area.py.
# For now, it is COMMENTED OUT so the file is not required to run this script.
#
# df <- read.csv("/data/ds340w/work/heaght_width_FULL.csv")
#
# df_processed <- df %>%
...
#     TRUE ~ as.character(pathology)  # If no match, keep as-is
#   ))
#
...
#   ) +
#   ggtitle(paste("Original Pixel Information for", formatted_total_dots,
#                 "CBIS-DDSM Mass Full Images \nBenign:", benign_count,
...
########################
# 2. CROPPED AREA PERCENTAGE (ACTIVE)
########################

# On the cluster, your CSV is here:
roi <- read.csv("/data/ds340w/work/598_percentage_all.csv")
...
# Print the p-value
print(paste("Wilcoxon p-value:", p_value_wilcox))
...
summary_stats <- roi_processed %>%
  ...
  summarise(
    ...
    # mean = mean(area_percentage),  # (optional) mean
...
ROI_area <- ggplot(roi_processed, aes(x = pathology_processed, y = area_percentage, color = pathology_processed)) +
  ...
  geom_hline(data = summary_stats, aes(yintercept = median, color = pathology_processed),
             linetype = "dashed", size = 0.5) +  # Add horizontal lines at medians
  ...
  ggtitle(paste(
    "Cropped Area Per 598 * 598 for", formatted_total_dots_ROI,
    "CBIS-DDSM Mass Images\nBenign:", benign_count,
    ", Malignant:", malignant_count
  ))
